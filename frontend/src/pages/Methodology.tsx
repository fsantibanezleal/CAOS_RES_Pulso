...
function PreprocSVG({ es }: { es: boolean }) {
  ...
  return (
    <Figure caption={es
      ? 'El preproceso: la presión cruda se remuestrea a una grilla log-uniforme, se deriva (Bourdet) y se normaliza a media cero y varianza uno, de modo que solo la forma sobrevive.'
...
function DtwGridSVG({ es }: { es: boolean }) {
  ...
  return (
    <Figure caption={es
      ? 'La matriz de alineación DTW: el camino óptimo (azul) serpentea para emparejar rasgos que ocurren en tiempos distintos; la banda de Sakoe-Chiba (sombreada) lo restringe a |i-j| <= w, dando invariancia a la escala de tiempo sin permitir alineaciones degeneradas.'
      ...
      <svg viewBox={`0 0 ${w} ${w}`} width="100%" style={{ maxWidth: 260 }} role="img"
        aria-label={es ? 'Rejilla de alineación DTW' : 'DTW alignment grid'}>
...
function InceptionSVG({ es }: { es: boolean }) {
  ...
  return (
    <Figure caption={es
      ? 'Un módulo Inception: un cuello de botella 1x1 seguido de convoluciones paralelas de kernel creciente (multi-escala) más una rama de max-pooling; las salidas se concatenan. Apilar estos módulos con atajos residuales cada 3 da el clasificador InceptionTime.'
      ...
      <svg viewBox="0 0 460 170" width="100%" style={{ maxWidth: 460 }} role="img"
        aria-label={es ? 'Módulo Inception' : 'Inception module'}>
...
function DualConformalSVG({ es }: { es: boolean }) {
  return (
    <Figure caption={es
      ? 'El conformal de doble representación: el conjunto de predicción es la intersección del conjunto conforme en forma (DTW) y el conforme en descriptores físicos (RF). Una curva de la zona amarilla (forma correcta) pero fuera del azul (física implausible) queda excluida: es "forma correcta, física incorrecta".'
      ...
      <svg viewBox="0 0 360 180" width="100%" style={{ maxWidth: 360 }} role="img"
        aria-label={es ? 'Conformal de doble representación' : 'Dual-representation conformal'}>
...
function Preprocessing({ es }: { es: boolean }) {
  return (
    <div>
      <p>{es
        ? 'Toda la escalera opera sobre la forma de la respuesta de presión, no sobre su amplitud absoluta. El primer paso convierte cada transiente crudo en una firma comparable. La presión adimensional de fondo se remuestrea a una grilla log-uniforme de tiempo (los ensayos abarcan varias décadas de tiempo, y la física de interés vive en el eje logarítmico), y sobre esa grilla se toma la derivada de Bourdet con una ventana de suavizado de L ciclos-log.'
      ...
      <p>{es
        ? 'La derivada suavizada se normaliza a media cero y varianza uno (z-score). Este paso hace que un pozo de alta permeabilidad y uno de baja permeabilidad con la misma física de fractura caigan en el mismo GeoType: la normalización elimina la escala vertical y deja solo el patrón de regímenes.'
        : 'The smoothed derivative is normalised to zero mean and unit variance (z-score). This step makes a high-permeability well and a low-permeability well with the same fracture physics fall into the same GeoType: normalisation removes the vertical scale and leaves only the regime pattern.'}</p>
      <Equation tex="\tilde x_i = \frac{p'_{wD}(t_{D,i}) - \mu}{\sigma}, \qquad \mu = \frac{1}{n}\sum_i p'_{wD}(t_{D,i}), \quad \sigma^2 = \frac{1}{n}\sum_i \big(p'_{wD}(t_{D,i})-\mu\big)^2"
        caption={es ? 'Ec. La normalización z-score sobre los n puntos de la grilla; el vector resultante es la firma que alimenta a DTW y a los modelos.' : 'Eq. The z-score normalisation over the n grid points; the resulting vector is the signature fed to DTW and the models.'} />
      <PreprocSVG es={es} />
      <p>{es
        ? 'Este preproceso es exactamente el que se ejecuta en vivo en el navegador (numpy/TS puro) cuando el usuario ajusta una curva sintética, y el mismo que se ejecutó offline al precalcular cada caso; por eso la asignación en vivo es idéntica a la comprometida.'
      ...
      <Callout variant="honest" title={es ? 'Suavizado, no invención' : 'Smoothing, not invention'}>
        {es ? 'La ventana L suaviza la derivada de datos ruidosos; una L demasiado grande borra el valle de doble porosidad y una demasiado pequeña lo llena de ruido. Se elige por caso y se documenta, y la curva cruda siempre está disponible en el explorador de ensamble.' : 'The window L smooths the derivative of noisy data; too large an L erases the dual-porosity valley and too small floods it with noise. It is chosen per case and documented, and the raw curve is always available in the ensemble explorer.'}
      </Callout>
...
function Distances({ es }: { es: boolean }) {
  return (
    <div>
      <p>{es
        ? 'Dos transientes con la misma sucesión de regímenes pero desfasados en el tiempo (por distinta permeabilidad o radio) deben considerarse similares. La distancia euclidiana punto a punto los penaliza por el desfase; el Dynamic Time Warping (DTW) los alinea bajo un emparejamiento monótono antes de medir, absorbiendo la escala de tiempo.'
      ...
      <Equation tex="D_{\mathrm{DTW}}(a,b) = \min_{\pi \in \Pi_w} \sum_{(i,j)\in\pi} \big(a_i - b_j\big)^2"
        caption={es ? 'Ec. La distancia DTW: el mínimo sobre todos los caminos de alineación monótonos pi dentro de la banda de ancho w.' : 'Eq. The DTW distance: the minimum over all monotonic alignment paths pi inside the band of width w.'} />
      <p>{es
        ? <>El mínimo se calcula por programación dinámica con la recurrencia de costo acumulado, restringida a la banda de Sakoe-Chiba <Cite id="sakoe1978" paren /> que prohíbe alineaciones degeneradas (un punto emparejado con muchos):</>
      ...
      <Equation tex="\gamma(i,j) = (a_i-b_j)^2 + \min\{\gamma(i-1,j),\,\gamma(i,j-1),\,\gamma(i-1,j-1)\}, \quad |i-j| \le w"
        caption={es ? 'Ec. La recurrencia de DTW con la restricción de banda |i-j| <= w. El backend offline es la implementación C de dtaidistance vía pygeotypes; el navegador reimplementa la misma recurrencia en TypeScript.' : 'Eq. The DTW recurrence with the band constraint |i-j| <= w. The offline backend is the C implementation in dtaidistance via pygeotypes; the browser reimplements the same recurrence in TypeScript.'} />
      <DtwGridSVG es={es} />
      <p>{es
      ...
      <Callout variant="honest" title={es ? 'Por qué no euclidiana' : 'Why not Euclidean'}>
        {es ? 'La comparación de métodos (pestaña Acuerdo de métodos en la App) incluye una línea base de k-medoids euclidiana y de correlación precisamente para mostrar cuándo la alineación DTW gana su costo y cuándo no: no se asume, se mide.' : 'The method comparison (Method agreement tab in the App) includes a Euclidean and a correlation k-medoids baseline precisely to show when the DTW alignment earns its cost and when it does not: it is not assumed, it is measured.'}
      </Callout>
...
function Catalogue({ es }: { es: boolean }) {
  return (
    <div>
      <p>{es
        ? 'El catálogo de GeoTypes se descubre por Partitioning Around Medoids (PAM) sobre la matriz DTW. PAM elige K medoides (curvas miembro reales) que minimizan la distancia total dentro del cluster; a diferencia de k-means, el prototipo de cada GeoType es una respuesta observada, no un promedio sintético que podría no corresponder a ninguna física real.'
      ...
      <Equation tex="\{m_1,\dots,m_K\} = \arg\min_{\{m_g\}} \sum_{i=1}^{n} \min_{g} D_{\mathrm{DTW}}(x_i, m_g)"
        caption={es ? 'Ec. El objetivo de PAM k-medoids: elegir K medoides que minimizan la distancia DTW total de cada curva a su medoide más cercano. Se resuelve con reinicios (n_init escala con el tamaño: 10 para <=800 curvas, menos para el benchmark).' : 'Eq. The PAM k-medoids objective: pick K medoids minimising the total DTW distance of each curve to its nearest medoid. Solved with restarts (n_init scales with size: 10 for <=800 curves, fewer for the benchmark).'} />
      <p>{es
        ? <>El número de GeoTypes K no se fija a mano: se barre un rango y se elige por el coeficiente de silueta <Cite id="kaufman1990" paren />, que mide cuán compacto y separado está cada cluster en la geometría DTW.</>
      ...
      <Equation tex="s(i) = \frac{b(i) - a(i)}{\max\{a(i),\,b(i)\}}, \qquad s = \frac{1}{n}\sum_i s(i)"
        caption={es ? 'Ec. La silueta: a(i) es la distancia media de la curva i a su propio cluster, b(i) la del cluster vecino más cercano. El K con mayor silueta media s es el elegido.' : 'Eq. The silhouette: a(i) is the mean distance of curve i to its own cluster, b(i) to the nearest neighbouring cluster. The K with the highest mean silhouette s is chosen.'} />
      <MedoidSVG es={es} />
      <p>{es
        ? 'La pestaña Predictibilidad-vs-K (Atribución+) muestra la silueta y la exactitud del bosque aleatorio a lo largo de K, para verificar que el K elegido no es solo geométricamente compacto sino también atribuible a descriptores físicos.'
...
function ClusteringComparison({ es }: { es: boolean }) {
  return (
    <div>
      <p>{es
        ? 'El catálogo DTW k-medoids es la referencia; la escalera lo mide honestamente contra las alternativas SOTA sobre los mismos datos. Cada método se evalúa por silueta (calidad en geometría DTW) y por Adjusted Rand Index (ARI, acuerdo corregido por azar con las etiquetas de referencia).'
...
function Representations({ es }: { es: boolean }) {
  return (
    <div>
      <p>{es
        ? 'Más allá de la geometría de distancias, el ensamble se dispone y describe con representaciones complementarias, todas alineadas curva-a-curva con los miembros comprometidos. El MDS clásico embebe la matriz DTW en 2D/3D preservando distancias; UMAP y t-SNE dan disposiciones de variedad (manifold) que a veces revelan estructura que el MDS lineal comprime.'
      ...
      <p>{es
        ? <>El PCA funcional descompone la matriz de curvas centrada por SVD; los modos propios son las formas dominantes de variación (interpretables), y las puntuaciones dan un scatter 2D <Cite id="ramsay2005" paren />. Las features catch22 <Cite id="lubba2019" paren /> reducen cada curva a 22 características canónicas, agregadas por cluster como una firma interpretable.</>
...
function Diagnostics({ es }: { es: boolean }) {
  return (
    <div>
      <p>{es
        ? 'La capa de diagnóstico clásica se ejecuta en vivo en el navegador (TypeScript puro, sin artefacto precalculado) sobre la curva que el usuario ajusta. Detecta y marca los regímenes de flujo en la derivada de Bourdet log-log por su pendiente característica, y ajusta modelos analíticos que recuperan los parámetros directamente de la curva.'
      ...
      <p>{es
        ? <>Las pendientes canónicas <Cite id="bourdet1989" paren />: flujo radial = meseta en 0.5 (pendiente 0), flujo lineal = 1/2, bilineal = 1/4, límite = pendiente unitaria. El valle de doble porosidad se detecta por valor (la derivada bien por debajo de 0.5), no por pendiente, porque sus flancos son empinados.</>
      ...
      <Equation tex="\hat\omega,\hat\lambda = \arg\min_{\omega,\lambda}\; \sum_i \big(\ln p_{wD,i} - \ln \tilde p_{wD}(t_{D,i};\omega,\lambda)\big)^2"
        caption={es ? 'Ec. El ajuste Warren-Root en vivo: recupera (omega, lambda) minimizando el residuo log-presión por una grilla gruesa + refinamiento local, barato por movimiento de slider.' : 'Eq. The live Warren-Root fit: recovers (omega, lambda) by minimising the log-pressure residual via a coarse grid + local refine, cheap per slider move.'} />
      <Equation tex="p_{wD}^{\text{Theis}}(t_D) = \tfrac12\,E_1\!\left(\tfrac{1}{4 t_D}\right) + S, \qquad E_1(u) = \int_u^{\infty}\frac{e^{-\tau}}{\tau}\,d\tau"
        caption={es ? 'Ec. El modelo homogéneo de Theis con skin S (integral exponencial E_1); el modelo de menor RMSE entre Warren-Root y Theis es el que sostienen los datos.' : 'Eq. The homogeneous Theis model with skin S (exponential integral E_1); the lower-RMSE model between Warren-Root and Theis is the one the data supports.'} />
      <Callout variant="honest" title={es ? 'Recuperar lo fijado' : 'Recover the set values'}>
        {es ? 'Que el ajuste recupere los valores que el usuario fijó en los sliders es la prueba honesta del ajustador: en una curva limpia recupera omega=0.049 vs 0.050 fijado (RMSE 0.0099), y Theis pierde en una curva de doble porosidad.' : 'That the fit recovers the values the user set on the sliders is the honest test of the fitter: on a clean curve it recovers omega=0.049 vs 0.050 set (RMSE 0.0099), and Theis loses on a dual-porosity curve.'}
      </Callout>
...
function LearnedClassifiers({ es }: { es: boolean }) {
  return (
    <div>
      <p>{es
        ? 'El nivel aprendido entrena arquitecturas SOTA en la GPU (torch cu124, .venv-train) y las exporta a ONNX con paridad < 1e-4, para ejecutarse en vivo en el navegador vía onnxruntime-web. Dos clasificadores forman un par CNN-vs-transformer sobre la misma tarea de GeoType.'
      ...
      <p>{es
        ? <>InceptionTime <Cite id="ismailfawaz2020" paren /> apila módulos Inception (cuello de botella 1x1 + convoluciones paralelas de kernel [9,19,39] + rama de pooling, concatenadas) con atajos residuales cada 3 y pooling promedio global; exactitud held-out 0.911.</>
      ...
      <Equation tex="\mathrm{Attention}(Q,K,V) = \mathrm{softmax}\!\left(\frac{Q K^{\top}}{\sqrt{d_k}}\right) V"
        caption={es ? 'Ec. La auto-atención escalada del encoder PatchTST: cada parche atiende a todos los parches, capturando dependencias de largo alcance en la forma del transiente.' : 'Eq. The scaled self-attention of the PatchTST encoder: each patch attends to all patches, capturing long-range dependencies in the transient shape.'} />
      <Equation tex="\hat y = \mathrm{softmax}\big(W\,\bar h + b\big), \qquad \bar h = \tfrac{1}{T}\textstyle\sum_t h_t \;\;(\text{InceptionTime, GAP})"
        caption={es ? 'Ec. La cabeza de clasificación: softmax sobre el pooling promedio global (InceptionTime) o el aplanado de parches (PatchTST). El softmax se aplica en la exportación, de modo que el navegador lee probabilidades directamente.' : 'Eq. The classification head: softmax over the global average pool (InceptionTime) or the flattened patches (PatchTST). Softmax is applied at export, so the browser reads probabilities directly.'} />
      <Callout variant="honest" title={es ? 'Herramientas, no tabla de métricas' : 'Tools, not a metrics table'}>
        {es ? 'Los modelos aprendidos aparecen como herramientas en vivo (clasifican esta curva), no como una tabla de métricas; la exactitud held-out se muestra junto a la predicción, y la paridad ONNX-torch se verifica antes de comprometer cada modelo.' : 'The learned models appear as live tools (classify this curve), not a metrics table; the held-out accuracy is shown next to the prediction, and the ONNX-torch parity is verified before each model is committed.'}
      </Callout>
...
function LearnedRetrieval({ es }: { es: boolean }) {
  return (
    <div>
      <p>{es
        ? 'El segundo par de modelos aprendidos no clasifica sino que detecta anomalías y recupera vecinos. Un autoencoder convolucional profundo comprime la curva a un latente y la reconstruye; el error de reconstrucción es una señal honesta de fuera-de-distribución: una curva distinta al catálogo de entrenamiento reconstruye mal.'
      ...
      <Equation tex="z = \mathrm{enc}(x), \quad \hat x = \mathrm{dec}(z), \qquad \mathcal{E}(x) = \tfrac{1}{N}\lVert x - \hat x\rVert_2^2"
        caption={es ? 'Ec. El autoencoder: el error cuadrático medio de reconstrucción E(x) es el puntaje de anomalía; el latente z (dim 8) es una incrustación 2D-proyectable del comportamiento.' : 'Eq. The autoencoder: the mean-squared reconstruction error E(x) is the anomaly score; the latent z (dim 8) is a 2D-projectable embedding of behaviour.'} />
      <p>{es
        ? <>El encoder estilo TS2Vec <Cite id="yue2022" paren /> es una red de convoluciones dilatadas entrenada contrastivamente: dos vistas enmascaradas de cada curva deben acercarse (positivas) frente a todas las demás (negativas), con la pérdida NT-Xent <Cite id="chen2020" paren />. Da una incrustación L2-normalizada para recuperación por vecino más cercano contra la nube de entrenamiento.</>
      ...
      <Equation tex="\mathcal{L}_{\text{NT-Xent}} = -\log \frac{\exp(\mathrm{sim}(z_i, z_i^+)/\tau)}{\sum_{k\neq i}\exp(\mathrm{sim}(z_i, z_k)/\tau)}, \qquad \tau = 0.2"
        caption={es ? 'Ec. La pérdida contrastiva NT-Xent: acerca las dos vistas enmascaradas de la misma curva y aleja las de otras, con temperatura tau=0.2. En inferencia el encoder es una pasada determinista, exportable a ONNX.' : 'Eq. The NT-Xent contrastive loss: pulls the two masked views of the same curve together and pushes others apart, with temperature tau=0.2. At inference the encoder is a deterministic forward pass, ONNX-exportable.'} />
      <Callout variant="honest" title={es ? 'La anomalía no es una etiqueta' : 'Anomaly is not a label'}>
        {es ? 'Un error de reconstrucción alto señala que la curva no se parece a nada del catálogo, no que sea "mala"; se combina con la bandera fuera-de-catálogo del conformal para una decisión robusta, y ninguno de los dos se presenta como verdad absoluta.' : 'A high reconstruction error signals that the curve resembles nothing in the catalogue, not that it is "bad"; it combines with the conformal out-of-catalogue flag for a robust decision, and neither is presented as absolute truth.'}
      </Callout>
...
function Conformal({ es }: { es: boolean }) {
  return (
    <div>
      <p>{es
        ? 'La asignación no da solo una etiqueta puntual sino un conjunto de predicción con garantía de cobertura. La predicción conformal condicional por clase (Mondrian) calibra, por cada GeoType, las distancias DTW de las curvas de calibración a su medoide; el p-valor de una curva nueva es su rango entre esas distancias.'
      ...
      <Equation tex="p_g(x) = \frac{1 + \#\{\, i : s_i^{(g)} \ge D_{\mathrm{DTW}}(x, m_g) \,\}}{1 + n_g}, \qquad \Gamma^{\alpha}(x) = \{\, g : p_g(x) > \alpha \,\}"
        caption={es ? 'Ec. El p-valor conformal de la curva x para el GeoType g y el conjunto de predicción al nivel 1-alpha. Un conjunto vacío es una bandera honesta de fuera-de-catálogo.' : 'Eq. The conformal p-value of curve x for GeoType g and the prediction set at level 1-alpha. An empty set is an honest out-of-catalogue flag.'} />
      <p>{es
        ? <>La cobertura marginal está garantizada por intercambiabilidad <Cite id="vovk2005" paren /><Cite id="angelopoulos2023" paren />: el conjunto contiene el GeoType verdadero con probabilidad al menos 1-alpha. Esta es la capa novel clásica de Pulso, y se ejecuta en vivo en el navegador.</>
      ...
      <p>{es
        ? 'Más allá de SOTA, la asignación de doble representación hace conformal conjuntamente sobre dos espacios: la forma (distancia DTW) y los descriptores físicos (implausibilidad del bosque aleatorio). El conjunto es la intersección por clase, de modo que una curva de forma correcta pero física implausible queda marcada, algo que el conformal de un solo puntaje no puede.'
...
function Attribution({ es }: { es: boolean }) {
  return (
    <div>
      <p>{es
        ? 'La atribución responde por qué: relaciona las etiquetas GeoType con los descriptores físicos de la red de fractura (intensidad, apertura, conectividad, fracción en el componente mayor) con un bosque aleatorio + SHAP, con un gate de exactitud honesto que retiene las importancias cuando el bosque no puede predecir las etiquetas.'
      ...
      <Equation tex="\phi_j = \sum_{S \subseteq F\setminus\{j\}} \frac{|S|!\,(|F|-|S|-1)!}{|F|!}\big[f(S\cup\{j\}) - f(S)\big]"
        caption={es ? 'Ec. El valor de Shapley phi_j del descriptor j: su contribución marginal promediada sobre todos los subconjuntos, calculado eficientemente por TreeSHAP sobre el bosque.' : 'Eq. The Shapley value phi_j of descriptor j: its marginal contribution averaged over all subsets, computed efficiently by TreeSHAP over the forest.'} />
      <p>{es
        ? <>El bosque aleatorio <Cite id="breiman2001" paren /> se entrena con 200 árboles y hoja mínima 2; el gate exige exactitud held-out {'>'} 0.7 (balanceada, para que los GeoTypes minoritarios cuenten). SHAP <Cite id="lundberg2020" paren /> da la importancia por clase, y el ROM barre cada descriptor para medir su sensibilidad de asignación.</>
      ...
      <Callout variant="honest" title={es ? 'Retener es un resultado' : 'Withholding is a result'}>
        {es ? 'Cuando el bosque no supera el gate (la atribución no es fiable), las importancias se retienen con una nota explícita en vez de reportarse como ruido; un gate cerca de 0.5 es información honesta, no una falla que ocultar.' : 'When the forest fails the gate (attribution is not reliable), the importances are withheld with an explicit note instead of being reported as noise; a gate near 0.5 is honest information, not a failure to hide.'}
      </Callout>
...
export function Methodology() {
  ...
  const tabs = [
    ...
    { id: 'catalogue', label: es ? '3. Catálogo k-medoids' : '3. k-medoids catalogue', content: <Catalogue es={es} /> },
    { id: 'compare', label: es ? '4. Comparación (SOTA)' : '4. Comparison (SOTA)', content: <ClusteringComparison es={es} /> },
    { id: 'reps', label: es ? '5. Representaciones' : '5. Representations', content: <Representations es={es} /> },
    { id: 'diag', label: es ? '6. Diagnósticos' : '6. Diagnostics', content: <Diagnostics es={es} /> },
    { id: 'learned', label: es ? '7. Clasificadores aprendidos' : '7. Learned classifiers', content: <LearnedClassifiers es={es} /> },
    { id: 'learned2', label: es ? '8. Anomalía + recuperación' : '8. Anomaly + retrieval', content: <LearnedRetrieval es={es} /> },
    { id: 'conformal', label: es ? '9. Asignación conformal' : '9. Conformal assignment', content: <Conformal es={es} /> },
    { id: 'attr', label: es ? '10. Atribución' : '10. Attribution', content: <Attribution es={es} /> },
  ];
  return (
    <div className="page-body prose">
      <div className="page-head">
        ...
        <p className="lede">
          {es
            ? 'La escalera de métodos, familia por familia: del preproceso y la distancia DTW al catálogo k-medoids, las alternativas SOTA de clustering, las representaciones, los diagnósticos en vivo, el nivel aprendido (GPU a ONNX) y la asignación conformal con atribución. Cada familia con su baseline clásico o SOTA y, donde es una capacidad del producto, una capa novel más allá de SOTA.'
