...
function ProtocolSVG({ es }: { es: boolean }) {
  return (
    <Figure caption={es
      ? 'El protocolo sin fugas: el ensamble se parte por una permutación semilla-fija en train / calibración / prueba. El catálogo se entrena solo con train; la calibración conformal solo con cal; la prueba es held-out. El anti-patrón (tachado) reutiliza las mismas curvas para entrenar y evaluar, lo que infla la cobertura.'
      ...
      <svg viewBox="0 0 620 200" width="100%" style={{ maxWidth: 620 }} role="img" aria-label={es ? 'Protocolo sin fugas' : 'Leakage-safe protocol'}>
        ...
        {[[es ? 'train -> catálogo' : 'train -> catalogue', 'var(--geo-0,#4f9cf9)'], [es ? 'cal -> conformal' : 'cal -> conformal', 'var(--geo-1,#f97b4f)'], [es ? 'test -> evaluar' : 'test -> evaluate', 'var(--geo-2,#41c98d)']].map(([l, c], i) => (
        ...
        <text x={10} y={112} fontSize={12} fill="currentColor">{es ? 'Anti-patrón (prohibido)' : 'Anti-pattern (forbidden)'}</text>
        <g opacity={0.6}>
...
function Protocol({ es }: { es: boolean }) {
  return (
    <div>
      <p>{es
        ? 'Cada estudio se evalúa bajo un protocolo sin fugas por construcción. Una permutación semilla-fija particiona el ensamble en tres slices disjuntos: entrenamiento (el catálogo k-medoids), calibración (los puntajes conformal condicionales por clase) y prueba (la evaluación held-out). El catálogo nunca ve las curvas de calibración ni de prueba.'
      ...
      <ProtocolSVG es={es} />
      <p>{es
        ? 'La garantía de cobertura conformal solo es válida si la calibración y la prueba son intercambiables y no vistas por el catálogo. Reutilizar curvas (el anti-patrón tachado) produce una cobertura inflada que no se sostiene fuera de muestra. Se dibuja el anti-patrón para dejar claro que no se hace.'
        : 'The conformal coverage guarantee is only valid if calibration and test are exchangeable and unseen by the catalogue. Reusing curves (the struck-out anti-pattern) produces an inflated coverage that does not hold out of sample. The anti-pattern is drawn to make clear it is not done.'}</p>
...
function Coverage({ es }: { es: boolean }) {
  return (
    <div>
      <p>{es
        ? 'La pregunta central de la asignación: ¿el conjunto de predicción conformal contiene el GeoType verdadero con la frecuencia prometida? La cobertura empírica en el slice de prueba debe acercarse al nivel objetivo 1-alpha; una desviación grande indicaría una violación de intercambiabilidad.'
      ...
      <Equation tex="\widehat{\text{cov}} = \frac{1}{n_{\text{test}}}\sum_{i=1}^{n_{\text{test}}} \mathbf{1}\!\big[\, g_i^\star \in \Gamma^{\alpha}(x_i) \,\big] \;\approx\; 1-\alpha"
        caption={es ? 'Ec. La cobertura empírica: la fracción de curvas de prueba cuyo GeoType verdadero cae en su conjunto de predicción. La tasa OOD es la fracción con conjunto vacío (fuera de catálogo).' : 'Eq. The empirical coverage: the fraction of test curves whose true GeoType falls in its prediction set. The OOD rate is the fraction with an empty set (out of catalogue).'} />
      <p>{es
        ? 'La pestaña Clasificar de la App muestra esto en vivo para una sola curva (p-valores, conjunto, bandera OOD); la tabla de Experimentos (pestaña Estudios) reporta la cobertura y la tasa OOD agregadas por caso, leídas del manifiesto comprometido.'
      ...
      <Callout variant="honest" title={es ? 'Cobertura, no exactitud' : 'Coverage, not accuracy'}>
        {es ? 'Un conjunto de predicción grande puede tener cobertura perfecta y aun así ser poco informativo; por eso también se reporta el tamaño medio del conjunto. La capa dual (Atribución+) intercambia un poco de cobertura por conjuntos más ajustados, y ambas cifras se muestran.' : 'A large prediction set can have perfect coverage yet be uninformative; that is why the mean set size is also reported. The dual layer (Attribution+) trades a little coverage for tighter sets, and both figures are shown.'}
      </Callout>
    </div>
  );
}
...
function MethodAgreementExp({ es }: { es: boolean }) {
  return (
    <div>
      <p>{es
        ? 'Segunda pregunta: ¿la estructura de GeoTypes es robusta al método, o un artefacto de DTW k-medoids? Se ejecutan 7 alternativas SOTA sobre el mismo ensamble y se mide su acuerdo con la referencia por Adjusted Rand Index. Donde muchos métodos coinciden, la estructura es real; donde solo uno lo hace, se lee con cautela.'
      ...
      <p>{es
        ? <>Resultado precalculado (pestaña Acuerdo de métodos de la App): en REAL_A, BENCH_A y BENCH_C, 6 de 7 métodos coinciden con DTW (ARI &gt; 0.5), y en BENCH_C HDBSCAN <Cite id="campello2013" paren /> recupera K por sí solo. Pero en BENCH_B (el dataset de backbone) solo el clustering espectral <Cite id="vonluxburg2007" paren /> coincide: la estructura ahí es frágil y se reporta como tal.</>
...
function Datasets({ es }: { es: boolean }) {
  return (
    <div>
      <p>{es
        ? 'Los datos que alimentan a Pulso, con su procedencia y términos de redistribución. Los datos sintéticos (familias analíticas, sims GeoDFN/open-DARTS) están etiquetados como tales; los reales (corpus 4TU, campañas de campo welltestpy) llevan su licencia. Nada restringido se re-hospeda.'
      ...
      <div className="scroll-x">
        <table>
          <thead><tr><th>{es ? 'dataset' : 'dataset'}</th><th>{es ? 'tipo' : 'type'}</th><th>{es ? 'redistribución' : 'redistribution'}</th><th>{es ? 'estado' : 'status'}</th></tr></thead>
          <tbody>
            ...
            <tr><td>GeoDFN + open-DARTS</td><td className="tag">{es ? 'sintético' : 'synthetic'}</td><td>{REDIST['GeoDFN + open-DARTS sims'][0]}</td><td><span className="badge ok">{REDIST['GeoDFN + open-DARTS sims'][1]}</span></td></tr>
            <tr><td>{es ? 'familias analíticas (Warren-Root)' : 'analytic families (Warren-Root)'}</td><td className="tag">{es ? 'sintético' : 'synthetic'}</td><td>{es ? 'generado en vivo' : 'generated live'}</td><td><span className="badge ok">live</span></td></tr>
          </tbody>
        </table>
      </div>
      <Callout variant="honest" title={es ? 'Sintético etiquetado' : 'Synthetic labelled'}>
        {es ? 'Cada celda sintética se marca; ningún dato sintético se presenta como medido. El corpus 4TU restringido no se re-hospeda: se usa su matriz DTW precomputada en el vault, y solo los resultados agregados se comprometen.' : 'Every synthetic cell is marked; no synthetic data is presented as measured. The restricted 4TU corpus is not re-hosted: its vault-only precomputed DTW matrix is used, and only aggregated results are committed.'}
      </Callout>
...
function Negatives({ es }: { es: boolean }) {
  return (
    <div>
      ...
      <ul>
        <li>{es ? 'BENCH_B: de 7 métodos de clustering, solo el espectral coincide con DTW; la estructura de GeoTypes ahí es frágil.' : 'BENCH_B: of 7 clustering methods, only spectral agrees with DTW; the GeoType structure there is fragile.'}</li>
        <li>{es ? 'Campañas de campo: la atribución cruzada entre sitios (Horkheim vs Lauswiesen) da un resultado nulo honesto; los descriptores no predicen el sitio, y la atribución se retiene.' : 'Field campaigns: cross-site attribution (Horkheim vs Lauswiesen) gives an honest null; the descriptors do not predict the site, and attribution is withheld.'}</li>
      ...
      <Callout variant="honest" title={es ? 'Un nulo es información' : 'A null is information'}>
        {es ? 'Cuando la atribución cae cerca del azar o un gate de fidelidad falla, se reporta explícitamente. Un producto que solo muestra éxitos no es honesto; el registro de experimentos, positivos y negativos, es el producto.' : 'When attribution falls near chance or a fidelity gate fails, it is reported explicitly. A product that only shows successes is not honest; the experiment record, positive and negative, is the product.'}
      </Callout>
    </div>
  );
}
...
function Studies({ es }: { es: boolean }) {
  ...
  return (
    <div>
      <p>{es ? 'Las métricas de cada estudio, leídas de los manifiestos comprometidos (nunca escritas a mano): K, silueta de entrenamiento, cobertura conformal y tasa fuera-de-catálogo.' : 'The metrics of every study, read from the committed manifests (never typed in): K, training silhouette, conformal coverage and out-of-catalogue rate.'}</p>
      <div className="scroll-x">
        <table>
...
export function Experiments() {
  ...
  const tabs = [
    ...
    { id: 'agreement', label: es ? '3. Acuerdo de métodos' : '3. Method agreement', content: <MethodAgreementExp es={es} /> },
  ...
  return (
    <div className="page-body prose">
      <div className="page-head">
        ...
        <p className="lede">
          {es
            ? 'Las preguntas experimentales, separadas: el protocolo sin fugas, la cobertura conformal, el acuerdo entre métodos, los datasets con su procedencia, la tabla de estudios (métricas de artefactos comprometidos) y los resultados negativos honestos. Números de artefactos, no escritos a mano.'
