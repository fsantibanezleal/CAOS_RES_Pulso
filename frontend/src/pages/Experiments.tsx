  ...
  return (
    <div>
      ...
      <Callout variant="honest" title={es ? 'Cobertura, no exactitud' : 'Coverage, not accuracy'}>
        {es ? 'Un conjunto de prediccion grande puede tener cobertura perfecta y aun así ser poco informativo; por eso también se reporta el tamaño medio del conjunto. La capa dual (Atribucion+) intercambia un poco de cobertura por conjuntos mas ajustados, y ambas cifras se muestran.' : 'A large prediction set can have perfect coverage yet be uninformative; that is why the mean set size is also reported. The dual layer (Attribution+) trades a little coverage for tighter sets, and both figures are shown.'}
      </Callout>
    </div>
  );
}
...
function MethodAgreementExp({ es }: { es: boolean }) {
  return (
    <div>
      <p>{es
        ? 'Segunda pregunta: la estructura de GeoTypes es robusta al método, o un artefacto de DTW k-medoids? Se corren 7 alternativas SOTA sobre el mismo ensamble y se mide su acuerdo con la referencia por Adjusted Rand Index. Donde muchos métodos coinciden, la estructura es real; donde solo uno lo hace, se lee con cautela.'
...
function Datasets({ es }: { es: boolean }) {
  return (
    <div>
      <p>{es
        ? 'Los datos que alimentan a Pulso, con su procedencia y terminos de redistribucion. Los datos sintéticos (familias analiticas, sims GeoDFN/open-DARTS) estan etiquetados como tales; los reales (corpus 4TU, campanas de campo welltestpy) llevan su licencia. Nada restringido se re-hospeda.'
...
function Negatives({ es }: { es: boolean }) {
  return (
    <div>
      ...
      <ul>
        <li>{es ? 'BENCH_B: de 7 métodos de clustering, solo el espectral coincide con DTW; la estructura de GeoTypes ahi es fragil.' : 'BENCH_B: of 7 clustering methods, only spectral agrees with DTW; the GeoType structure there is fragile.'}</li>
  ...
  return (
    <div className="page-body prose">
      <div className="page-head">
        ...
        <p className="lede">
          {es
            ? 'Las preguntas experimentales, separadas: el protocolo sin fugas, la cobertura conformal, el acuerdo entre métodos, los datasets con su procedencia, la tabla de estudios (metricas de artefactos comprometidos) y los resultados negativos honestos. Numeros de artefactos, no escritos a mano.'
