...
export const ARCH_TABS: ArchTab[] = [
  {
    ...
    body_en:
      ...
      'It works on any ensemble in this class: user-supplied discrete-fracture-network + flow simulations, real field pumping-test campaigns, or community benchmark corpora.',
    body_es:
      'Pulso cataloga comportamientos de flujo de transitorios de presión sobre ensambles de redes de fracturas. Un ensayo de pozo perturba un reservorio o acuífero y la respuesta de presión (un pulso de presión) lleva la geometría de los caminos de flujo en su forma. Pulso toma un ensamble de esas respuestas, descubre los tipos de comportamiento recurrentes con aprendizaje no supervisado, atribuye cada tipo a las propiedades de la red que lo controlan, y asigna curvas nuevas con una garantía de cobertura.\n\n' +
      'Funciona con cualquier ensamble de esta clase: simulaciones propias de redes discretas de fracturas + flujo, campañas reales de ensayos de bombeo, o corpus de referencia comunitarios.',
  ...
  },
  {
    ...
    body_es:
      'Tres carriles de cómputo. offline (pesado, precomputado): GeoDFN genera redes geológicamente consistentes, open-DARTS simula sus transitorios de presión, las distancias de forma (DTW) y el agrupamiento (k-medoids y la escalera más amplia) construyen el catálogo, y los modelos aprendidos se entrenan en la GPU. live (en el navegador): los diagnósticos clásicos y las herramientas de forma/asignación se ejecutan en un motor TypeScript, y los modelos aprendidos se ejecutan con onnxruntime-web sobre el ONNX exportado offline. replay: los artefactos del ensamble, decimados y comprometidos, alimentan cada vista.\n\n' +
      'Nada pesado se ejecuta en un servidor; el sitio es estático. El carril offline precalcula los artefactos; el navegador los reproduce y ejecuta la inferencia aprendida en vivo.',
  ...
  },
  {
    ...
    body_es:
      'El build superpone los artefactos comprometidos (data/derived, models) en el paquete estático. La SPA React carga un caso de ensamble, lee su artefacto de ensamble completo (cada curva decimada, etiquetas de cluster, geometrías de red, los embeddings DTW/MDS, tablas de atribución, calibración conforme), y renderiza el workbench: un explorador de curvas del ensamble, paneles por cluster con miniaturas de red, distribuciones de propiedades, el mapa del espacio de formas, la estabilidad entre ensambles, el estudio de atribución, y el banco de diagnóstico. Cada vista está enlazada por brushing y reacciona al caso seleccionado y a los controles.',
  ...
  },
  {
    ...
    body_es:
      'El núcleo científico es una escalera profunda de métodos, de clásico a SOTA. Diagnósticos: derivada de Bourdet + detección de regímenes de flujo, ajustes Warren-Root y Theis, deconvolución. Distancias y agrupamiento: DTW + PAM (FasterPAM), baricentros soft-DTW, k-Shape, jerárquico, espectral, HDBSCAN. Representaciones: MDS clásico, UMAP, t-SNE, PCA funcional, features catch22. Aprendidos (GPU a ONNX): un clasificador InceptionTime, un autoencoder convolucional para puntuar fuera de distribución, un encoder contrastivo para recuperación, un transformer de parches. Atribución: Random Forest + SHAP con un gate de exactitud. Asignación: predicción split-conforme (y una variante condicional por clase).',
  ...
  },
  {
    ...
    body_es:
      'Dos contratos mantienen el pipeline offline y la web en sincronía. CONTRACT-1 valida las curvas de entrada (finitas, tiempo positivo, rango de respuesta real) antes de precalcular nada. CONTRACT-3 es el artefacto de ensamble completo comprometido por caso: cada curva miembro (decimada min/max por pixel), etiquetas de cluster y envolventes de cuantiles por cluster, geometrías de red decimadas con sus propiedades (incluido el tipo de set), la matriz DTW ordenada por cluster, las coordenadas MDS 2D/3D, las tablas de atribución, y la calibración conforme. Un espejo TypeScript del esquema hace que cualquier desvío rompa el build.',
